#!/usr/bin/env python3
"""
Build Section Cuivres — HTML + graphs from CSV v22 (single source of truth)
"""
import matplotlib
matplotlib.use('Agg')
...
def generate_bar_chart(display_name, filename, n, formants, fp=None, description=""):
    ...
    for i, freq in valid:
        ...
        ax.bar(freq, bar_h, width=bw * (1.2 - i * 0.05),
               color=FORMANT_COLORS[i], alpha=FORMANT_ALPHAS[i],
    ...
    ax.text(485, 0.02, 'cluster /o/', ha='center', va='bottom',
            fontsize=7, color='#C62828', fontstyle='italic', transform=ax.get_xaxis_transform())
    
    ax.set_xlim(100, max_freq); ax.set_ylim(0, 1.25)
    ...
    ax.set_title(f"{display_name}  —  Formants spectraux F1–F6 (ordinario, N={n})",
    ...
    legend_elements = [
        mpatches.Patch(facecolor=FORMANT_COLORS[i], alpha=FORMANT_ALPHAS[i],
                       edgecolor='#333', label=f'F{i+1} = {formants[i]} Hz')
        for i, f in valid
    ]
    if fp and abs(fp - f1) > 30:
        legend_elements.append(Line2D([0],[0], marker='D', color='w', markerfacecolor='#1B5E20',
    ...
    ax.text(0.01, -0.08, f"Famille : Cuivres  |  Source : formants_all_techniques.csv (SOL2020 v22)",
...
DESCRIPTIONS = {
    ...
    ('Trombone+sordina_harmon', 'ordinario'): "Son très concentré, quasi sinusoïdal dans le grave (F1=162 Hz). Timbre « de jazz » intime, projection très directionnelle.",
...
# Define all instruments to process
INSTRUMENTS = [
    # (csv_name, display_name, graph_filename, fp)
...
# Map special techniques for wah
TECH_OVERRIDE = {
...
for csv_name, display, gfx_name, fp in INSTRUMENTS:
    ...
    graph_paths[gfx_name] = img_path
    all_data[gfx_name] = {'name': display, 'csv': csv_name, 'tech': tech, 
                           'data': d, 'fp': fp, 'img': img_path}
    desc_key = (csv_name, tech)
    desc = DESCRIPTIONS.get(desc_key, "")
    all_data[gfx_name]['description'] = desc
    print(f"  ✓ {display:<35s} N={d['n']:>4d}  F1={d['F'][0]:>5d}  → {img_path}")
...
def make_technique_table(inst_csv_name):
    """Build HTML table for all techniques of an instrument"""
    techs = sorted([(t, DATA[(i,t)]) for (i,t) in DATA if i == inst_csv_name], 
                   key=lambda x: x[0])
    if not techs: return ""
...
def instrument_section(gfx_key, show_all_techniques=True):
    """Generate HTML for one instrument"""
    ...
    tech_table = make_technique_table(info['csv']) if show_all_techniques else ''
    
    return f"""
    <div class="instrument-card">
      <h3>{info['name']}</h3>
      <img src="{info['img']}" alt="{info['name']}" class="formant-graph"/>
      {desc_html}
      {fp_html}
      {tech_table}
    </div>"""
...
<div class="section-intro">
  ...
  Cor anglais et Saxophone ténor — fondement acoustique des doublures classiques.</p>
  <p><strong>Source unique :</strong> toutes les valeurs proviennent du CSV <code>formants_all_techniques.csv</code>,
  extrait par le script v22 (<code>extract_formants_all_techniques_v2_fixed.py</code>) à partir des specenv SOL2020/Yan_Adds.
  Pipeline validé : 16/16 instruments reproduits à Δ=0 Hz.</p>
</div>
...
for key in ['cuivres_bass_tuba_ord', 'cuivres_horn_ord', 'cuivres_trombone_ord', 'cuivres_trumpet_c_ord']:
    if key in all_data:
        html += instrument_section(key, show_all_techniques=True)

html += "\n<h2>Instruments additionnels (Yan_Adds)</h2>\n"

for key in ['cuivres_bass_trombone_ord', 'cuivres_contrabass_tuba_ord']:
    if key in all_data:
        html += instrument_section(key, show_all_techniques=True)

html += "\n<h2>Cuivres avec sourdine</h2>\n"
html += """<div class="section-intro">
  <p>Les sourdines modifient profondément le spectre formantique des cuivres. 
  L'effet varie considérablement selon le type de sourdine : 
  la <em>cup</em> atténue les aigus (son feutré), 
  la <em>straight</em> renforce les médiums (son nasal), 
  la <em>harmon</em> concentre l'énergie (son « jazz »), 
  et la <em>wah</em> permet un contrôle dynamique ouvert/fermé.</p>
</div>\n"""

# Sordina sections  
sordina_order = [
    'cuivres_trb_sord_cup', 'cuivres_trb_sord_straight', 'cuivres_trb_sord_harmon',
    'cuivres_trb_sord_wah_open', 'cuivres_trb_sord_wah_closed',
    'cuivres_tpt_sord_cup', 'cuivres_tpt_sord_straight', 'cuivres_tpt_sord_harmon',
    'cuivres_tpt_sord_wah_open', 'cuivres_tpt_sord_wah_closed',
    'cuivres_horn_sord', 'cuivres_bass_tuba_sord',
]

for key in sordina_order:
    if key in all_data:
        html += instrument_section(key, show_all_techniques=False)
...
<p class="source-note">
  ...
  <strong>Échantillons :</strong> SOL2020 (IRCAM) + Yan_Adds — technique ordinario uniquement sauf mention contraire.
</p>
</body>
</html>"""

out_html_path = os.path.join(OUT_HTML, "section_cuivres.html")
with open(out_html_path, 'w', encoding='utf-8') as f:
    f.write(html)

print(f"\n{'='*60}")
print(f"HTML: {out_html_path}")
print(f"Images: {len(graph_paths)} graphiques dans {OUT_IMG}/")
