#!/usr/bin/env python3
"""
Build Sections Saxophones + Cuivres — HTML + graphs from CSV v22
"""
import matplotlib
matplotlib.use('Agg')
...
# ─── Graph engine (identical to bois) ───
VOWEL_ZONES = [
...
def make_graph(display, filename, n, formants, fp=None, family_color='#E65100'):
    ...
    ax.text(0.01,-0.08, f"Source : CSV v22 (SOL2020 + Yan_Adds)",
...
DESC = {
    ...
    ('Trumpet_C','ordinario'): "Son brillant et incisif, grande projection. F1=786 Hz dans la zone /a/ (puissance). Fp centroïde à 1046 Hz remarquablement stable (σ=98) malgré la très forte variabilité de F2 (σ=1018).",
...
# ═══════════════════════════════════════════════════════════
SAXOPHONES = [
    ('Sax_Alto', 'Saxophone alto', 'sax_alto', 'ordinario'),
]

CUIVRES = [
    # Cor
    ('Horn', 'Cor en Fa', 'cuivres_horn', 'ordinario'),
    ('Horn+sordina', 'Cor — sourdine', 'cuivres_horn_sord', 'ordinario'),
    # Trompette
    ('Trumpet_C', 'Trompette en Do', 'cuivres_trumpet_c', 'ordinario'),
    ('Trumpet_C+sordina_straight', 'Trompette — sourdine straight (sèche)', 'cuivres_tpt_sord_straight', 'ordinario'),
    ('Trumpet_C+sordina_cup', 'Trompette — sourdine cup (bol)', 'cuivres_tpt_sord_cup', 'ordinario'),
    ('Trumpet_C+sordina_wah', 'Trompette — sourdine wah open', 'cuivres_tpt_sord_wah_open', 'ordinario_open'),
    ('Trumpet_C+sordina_wah', 'Trompette — sourdine wah closed', 'cuivres_tpt_sord_wah_closed', 'ordinario_closed'),
    ('Trumpet_C+sordina_harmon', 'Trompette — sourdine harmon', 'cuivres_tpt_sord_harmon', 'ordinario'),
    # Trombone
    ('Trombone', 'Trombone ténor', 'cuivres_trombone', 'ordinario'),
    ('Trombone+sordina_straight', 'Trombone — sourdine straight (sèche)', 'cuivres_trb_sord_straight', 'ordinario'),
    ('Trombone+sordina_cup', 'Trombone — sourdine cup (bol)', 'cuivres_trb_sord_cup', 'ordinario'),
    ('Trombone+sordina_wah', 'Trombone — sourdine wah open', 'cuivres_trb_sord_wah_open', 'ordinario_open'),
    ('Trombone+sordina_wah', 'Trombone — sourdine wah closed', 'cuivres_trb_sord_wah_closed', 'ordinario_closed'),
    ('Trombone+sordina_harmon', 'Trombone — sourdine harmon', 'cuivres_trb_sord_harmon', 'ordinario'),
    # Tubas
    ('Bass_Tuba', 'Tuba basse', 'cuivres_bass_tuba', 'ordinario'),
    ('Bass_Tuba+sordina', 'Tuba basse — sourdine', 'cuivres_bass_tuba_sord', 'ordinario'),
    ('Bass_Trombone', 'Trombone basse', 'cuivres_bass_trombone', 'ordinario'),
    ('Contrabass_Tuba', 'Tuba contrebasse', 'cuivres_contrabass_tuba', 'ordinario'),
]
...
def card(gfx, show_techs=True):
    ...
    if not info: return ""
    d = info['data']; desc = DESC.get((info['csv'], info['tech']), '')
...
<div class="section-intro sax">
  <p><strong>Note :</strong> Seul le saxophone alto est présent dans la base SOL2020. 
  Les saxophones ténor et baryton, mentionnés dans la littérature (Giesler cite le ténor 
  dans le cluster /o/), ne disposent pas de données specenv dans le corpus actuel.</p>
</div>
"""

html += card('sax_alto', show_techs=True)
...
<div class="section-intro cuivres">
  ...
  <p><strong>Caractéristique :</strong> formants bien définis, grande stabilité inter-dynamiques.
  Le <em>cluster de convergence</em> 450–502 Hz (zone /o/) rassemble Cor, Trombone, Basson, 
  Violoncelle, Cor anglais et Saxophone ténor.</p>
</div>

<h2 class="cuivres">Cor</h2>
"""

html += card('cuivres_horn', True)
html += card('cuivres_horn_sord', False)

html += '\n<h2 class="cuivres">Trompette</h2>\n'
html += card('cuivres_trumpet_c', True)
for k in ['cuivres_tpt_sord_straight','cuivres_tpt_sord_cup','cuivres_tpt_sord_wah_open',
           'cuivres_tpt_sord_wah_closed','cuivres_tpt_sord_harmon']:
    html += card(k, False)

html += '\n<h2 class="cuivres">Trombone</h2>\n'
html += card('cuivres_trombone', True)
for k in ['cuivres_trb_sord_straight','cuivres_trb_sord_cup','cuivres_trb_sord_wah_open',
           'cuivres_trb_sord_wah_closed','cuivres_trb_sord_harmon']:
    html += card(k, False)

html += '\n<h2 class="cuivres">Tubas</h2>\n'
for k in ['cuivres_bass_tuba','cuivres_bass_tuba_sord','cuivres_bass_trombone','cuivres_contrabass_tuba']:
    show = k in ('cuivres_bass_tuba','cuivres_bass_trombone','cuivres_contrabass_tuba')
    html += card(k, show)

# ─── Mute summary table ───
html += """
<h2 class="cuivres">Effet des sourdines sur les formants</h2>
<div class="mute-summary">
  <p>Tableau comparatif : F1 ordinario sans sourdine vs avec chaque type de sourdine.</p>
  <table>
    <tr><th>Instrument</th><th>Sans sourdine</th><th>Straight</th><th>Cup</th><th>Wah open</th><th>Wah closed</th><th>Harmon</th></tr>
"""

mute_data = [
    ('Trompette Do', 
     all_info.get('cuivres_trumpet_c',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_tpt_sord_straight',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_tpt_sord_cup',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_tpt_sord_wah_open',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_tpt_sord_wah_closed',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_tpt_sord_harmon',{}).get('data',{}).get('F',[0])[0]),
    ('Trombone ténor',
     all_info.get('cuivres_trombone',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_trb_sord_straight',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_trb_sord_cup',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_trb_sord_wah_open',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_trb_sord_wah_closed',{}).get('data',{}).get('F',[0])[0],
     all_info.get('cuivres_trb_sord_harmon',{}).get('data',{}).get('F',[0])[0]),
]
...
# Cor and tuba (only one mute type each)
...
html += """  </table>
  <p><em>Observations : la sourdine harmon produit les déplacements les plus extrêmes 
  (Trombone F1 chute à 162 Hz, Trompette F1 monte à 2358 Hz). La sourdine wah fermée 
  comprime les formants vers le medium. La sourdine cup atténue les aigus de manière 
  homogène.</em></p>
</div>
"""
...
html += """
<p class="source-note">
  <strong>Source des données :</strong> formants_all_techniques.csv — pipeline v22 validé (Δ=0 Hz).<br/>
  <strong>Instruments Yan_Adds :</strong> Trombone basse, Tuba contrebasse.
</p>
</body>
</html>"""

with open(os.path.join(OUT_HTML, "section_saxophones_cuivres.html"), 'w', encoding='utf-8') as f:
    f.write(html)

print(f"\n{'='*60}")
print(f"HTML: section_saxophones_cuivres.html")
