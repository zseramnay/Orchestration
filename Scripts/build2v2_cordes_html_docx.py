#!/usr/bin/env python3
'''
Build Section Cordes v2 — HTML + DOCX + graphs from CSV v2 (single source of truth)
...
def load_data(path):
    with open(path, 'r', encoding='utf-8') as f:
        for row in csv.DictReader(f):
            ...
            # Dual keying keeps both legacy and v2 names functional.
...
'''
    ...
    html += '''
<p class="source-note">
<strong>Source des données :</strong> formants_all_techniques_v2.csv + formants_yan_adds_v2.csv (fallback legacy si absent) — pipeline v22 validé.<br/>
...
html_path = os.path.join(OUT_DIR, 'section_cordes_v2.html')
docx_path = os.path.join(OUT_DIR, 'section_cordes_v2.docx')
