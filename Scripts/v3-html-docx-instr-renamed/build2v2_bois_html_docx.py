#!/usr/bin/env python3
"""
Build Section Bois v2 — HTML + DOCX + graphs from CSV v2 (single source of truth)
"""
import matplotlib
matplotlib.use('Agg')
...
V2_TO_LEGACY = {v: k for k, v in LEGACY_TO_V2.items()}
...
def load_data(path):
    with open(path, 'r', encoding='utf-8') as f:
        for row in csv.DictReader(f):
            ...
            # Keep dual keys so this script stays robust with mixed naming.
            if inst in LEGACY_TO_V2:
                DATA[(LEGACY_TO_V2[inst], tech)] = row
            if inst in V2_TO_LEGACY:
                DATA[(V2_TO_LEGACY[inst], tech)] = row


csv_all = 'Resultats/formants_all_techniques_v2.csv'
csv_yan = 'Resultats/formants_yan_adds_v2.csv'
if not os.path.exists(csv_all):
    csv_all = 'Resultats/formants_all_techniques.csv'
if not os.path.exists(csv_yan):
    csv_yan = 'Resultats/formants_yan_adds.csv'
...
# ─── Fp from audit ───
FP = {
    'Petite flûte': 876, 'Flûte': 1535, 'Flûte basse': 1338, 'Flûte contrebasse': 1092,
    'Hautbois': 1485, 'Cor anglais': 1135, 'Clarinette en Mib': 1266, 'Clarinette en Sib': 1412,
    'Clarinette basse en Sib': 1204, 'Clarinette contrebasse en Sib': 1090, 'Basson': 1079,
    'Contrebasson': 1279,
}
...
# ─── Descriptions ───
DESC = {
    'Petite flûte': "Son extrêmement brillant et perçant. F1=1109 Hz, déjà dans la zone /a/ (puissance). Le piccolo est l'instrument le plus aigu de l'orchestre — ses formants se situent intégralement au-dessus de 1000 Hz. Fp centroïde à 876 Hz capture la zone de résonance de l'embouchure.",
    'Flûte': "Son pur et aérien, riche en fondamentale. F1=743 Hz dans la zone /å/ (transition). Le Fp centroïde à 1535 Hz reflète la zone de résonance du tuyau. Grande variabilité de F2 par registre (σ=375 Hz).",
    'Flûte basse': "Son chaud et soufflé, plus riche en harmoniques que la flûte standard. F1=301 Hz dans la zone /u/ (profondeur). Timbre velouté avec une composante d'air caractéristique.",
    'Flûte contrebasse': "Son très grave et breathy, à la limite du souffle perceptible. F1=334 Hz (zone /u/). Timbre profond et enveloppant, souvent utilisé pour ses qualités texturales.",
    'Hautbois': "Son nasal et pénétrant, grande projection. F1=743 Hz (zone /å/), Fp=1485 Hz. L'anche double produit un spectre riche en harmoniques pairs et impairs, avec une zone formantique caractéristique autour de 1000-1200 Hz (Meyer).",
    'Cor anglais': "Son plus sombre et mélancolique que le hautbois. F1=452 Hz tombe dans le cluster /o/ (450-502 Hz) — ce qui explique sa fusion naturelle avec cor, basson et violoncelle. Fp=1135 Hz.",
    'Clarinette en Mib': "Son brillant et incisif, plus perçant que la clarinette Sib. F1=678 Hz (zone /å/). La clarinette Mib possède le spectre le plus aigu de la famille des clarinettes, avec F2=1540 Hz.",
    'Clarinette en Sib': "Son chaud dans le chalumeau, brillant dans le clairon. F2 extrêmement variable (σ=795 Hz) : de 549 Hz (chalumeau) à 2638 Hz (aigu). Le Fp centroïde à 1412 Hz est 4.7× plus stable que F2. Spectre dominé par les harmoniques impairs (tuyau cylindrique).",
    'Clarinette basse en Sib': "Son profond et velouté, riche et soyeux dans le grave. F1=323 Hz (zone /u/). Le registre chalumeau possède une couleur très distinctive, utilisée pour ses qualités expressives et mystérieuses.",
    'Clarinette contrebasse en Sib': "Son extrêmement grave, puissant et bourdonnant. F1=323 Hz identique à la clarinette basse, mais F2=937 Hz montre une résonance plus développée dans le medium. Timbre massif et enveloppant.",
    'Basson': "Son chaud et expressif, grande flexibilité timbrale. F1=495 Hz au cœur du cluster /o/ (450-502 Hz). Le basson est le pivot timbral de l'orchestre : Δ=3 Hz avec le violoncelle, Δ=45 Hz avec le cor. Fp=1079 Hz.",
    'Contrebasson': "Son très grave et bourdonnant, fondation des bois graves. F1=226 Hz (zone /u/), identique au tuba basse. Fp=1279 Hz. Technique analysée : non-vibrato (pas d'ordinario disponible dans la base).",
}
...
# ═══════════════════════════════════════════════════════════
BOIS = [
    ('Petite flûte', 'Petite flûte', 'bois_piccolo', 'ordinario'),
    ('Flûte', 'Flûte', 'bois_flute', 'ordinario'),
    ('Flûte basse', 'Flûte basse', 'bois_bass_flute', 'ordinario'),
    ('Flûte contrebasse', 'Flûte contrebasse', 'bois_contrabass_flute', 'ordinario'),
    ('Hautbois', 'Hautbois', 'bois_oboe', 'ordinario'),
    ('Cor anglais', 'Cor anglais', 'bois_english_horn', 'ordinario'),
    ('Clarinette en Mib', 'Clarinette en Mib', 'bois_clarinet_eb', 'ordinario'),
    ('Clarinette en Sib', 'Clarinette en Sib', 'bois_clarinet_bb', 'ordinario'),
    ('Clarinette basse en Sib', 'Clarinette basse en Sib', 'bois_bass_clarinet', 'ordinario'),
    ('Clarinette contrebasse en Sib', 'Clarinette contrebasse en Sib', 'bois_contrabass_clarinet', 'ordinario'),
    ('Basson', 'Basson', 'bois_bassoon', 'ordinario'),
    ('Contrebasson', 'Contrebasson', 'bois_contrabassoon', 'non-vibrato'),
]
...
"""
    ...
    for key in ['bois_piccolo', 'bois_flute', 'bois_bass_flute', 'bois_contrabass_flute']:
        ...
        is_yan = info['csv'] in ('Petite flûte', 'Flûte basse', 'Flûte contrebasse')
...
{tech_table_html(info['csv'])}
    ...
    for key in ['bois_oboe', 'bois_english_horn', 'bois_bassoon', 'bois_contrabassoon']:
        ...
        if not info:
            continue
        desc = DESC.get(info['csv'], '')
        is_yan = info['csv'] in ('Cor anglais', 'Contrebasson')
        badge = '<span class="yan-badge">Yan_Adds</span>' if is_yan else ''
        fp_html = f'<p class="fp-note">Fp centroïde = {info["fp"]} Hz</p>' if info['fp'] else ''
        tech_note = "<p><em>⚠ Contrebasson : technique analysée = non-vibrato (pas d'ordinario dans la base).</em></p>" if info['csv'] == 'Contrebasson' else ''
        html += f"""
<div class=\"instrument-card\">
<h3>{info['display']}{badge}</h3>
<img src=\"{info['img_rel']}\" alt=\"{info['display']}\" class=\"formant-graph\"/>
...
{tech_table_html(info['csv'])}
    ...
    for key in ['bois_clarinet_eb', 'bois_clarinet_bb', 'bois_bass_clarinet', 'bois_contrabass_clarinet']:
        ...
        if not info:
            continue
        desc = DESC.get(info['csv'], '')
        is_yan = info['csv'] in ('Clarinette en Mib', 'Clarinette basse en Sib', 'Clarinette contrebasse en Sib')
        badge = '<span class="yan-badge">Yan_Adds</span>' if is_yan else ''
        fp_html = f'<p class="fp-note">Fp centroïde = {info["fp"]} Hz</p>' if info['fp'] else ''
        html += f"""
<div class=\"instrument-card\">
...
def add_instrument(doc, info):
    ...
    title = info['display']
    if info['csv'] in ('Petite flûte', 'Flûte basse', 'Flûte contrebasse', 'Cor anglais', 'Clarinette en Mib', 'Clarinette basse en Sib', 'Clarinette contrebasse en Sib', 'Contrebasson'):
        title += ' [Yan_Adds]'
...
def build_docx(output_path):
    ...
    p = doc.add_paragraph()
    notes = [
        'Source des données : formants_all_techniques_v2.csv (fallback formants_all_techniques.csv si absent) — pipeline v22 validé (Δ=0 Hz pour 16/16 instruments CSV).',
        ...
        'Instruments Yan_Adds : Petite flûte, Flûte basse, Flûte contrebasse, Cor anglais, Clarinette Mib, Clarinette basse, Clarinette contrebasse, Contrebasson.',
    ]
...
html_path = os.path.join(OUT_DIR, 'section_bois_v2.html')
docx_path = os.path.join(OUT_DIR, 'section_bois_v2.docx')
