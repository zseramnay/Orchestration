...
def make_fig2_bark():
    """Fig 2b — Espace vocalique F1/F2 en Bark (Traunmüller), anti-collision labels."""
    from adjustText import adjust_text
    ...
    bark_ticks_x = [100, 200, 300, 400, 500, 700, 1000, 1500]
    ax.set_xticks([hz_to_bark(h) for h in bark_ticks_x])
    ax.set_xticklabels([f'{h}\n({hz_to_bark(h):.1f})' for h in bark_ticks_x], fontsize=7)
    bark_ticks_y = [200, 400, 600, 800, 1000, 1500, 2000, 3000]
    ax.set_yticks([hz_to_bark(h) for h in bark_ticks_y])
    ax.set_yticklabels([f'{h} ({hz_to_bark(h):.1f})' for h in bark_ticks_y], fontsize=7)
