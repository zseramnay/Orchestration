#!/usr/bin/env python3
# Build Sections Saxophones + Cuivres v2 — HTML + DOCX + graphs from CSV v2
import matplotlib
matplotlib.use('Agg')
...
<p class="source-note"><strong>Source des données :</strong> formants_all_techniques_v2.csv + formants_yan_adds_v2.csv (fallback legacy si absent) — pipeline v22 validé (Δ=0 Hz).<br/><strong>Instruments Yan_Adds :</strong> Trombone basse, Tuba contrebasse.</p>
</body>
</html>'''
...
def build_docx(output_path):
    ...
    p = doc.add_paragraph()
    notes = ['Source des données : formants_all_techniques_v2.csv + formants_yan_adds_v2.csv (fallback legacy si absent) — pipeline v22 validé (Δ=0 Hz).', 'Instruments Yan_Adds : Trombone basse, Tuba contrebasse.']
    for i, line in enumerate(notes):
        run = p.add_run(line + ('\n' if i < len(notes) - 1 else ''))
        run.font.size = Pt(9)
    doc.save(output_path)

html_path = os.path.join(OUT_DIR, 'section_saxophones_cuivres_v2.html')
docx_path = os.path.join(OUT_DIR, 'section_saxophones_cuivres_v2.docx')
