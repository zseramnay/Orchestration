#!/usr/bin/env python3
# Build synthesis sections v2 + DOCX export
import csv
import os
import shutil
import matplotlib
...
<p class="source-note">Document généré à partir de formants_all_techniques_v2.csv et formants_yan_adds_v2.csv (fallback legacy si absent).</p>
</body></nhtml>'''
...
def build_docx(output_path):
    ...
    p = doc.add_paragraph(); r = p.add_run('Document généré à partir de formants_all_techniques_v2.csv et formants_yan_adds_v2.csv (fallback legacy si absent).'); r.font.size = Pt(9)
    doc.save(output_path)

html_path = os.path.join(OUT_DIR, 'section_synthese_v2.html')
docx_path = os.path.join(OUT_DIR, 'section_synthese_v2.docx')
