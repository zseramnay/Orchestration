#!/usr/bin/env python3
# Build Section Cuivres v2 — HTML + DOCX + graphs from CSV v2 (single source of truth)
import matplotlib
matplotlib.use('Agg')
...
def generate_bar_chart(display_name, filename, n, formants, fp=None):
    ...
    ax.text(0.01, -0.08, 'Famille : Cuivres\nSource : formants_all_techniques_v2.csv (+ fallback legacy)', transform=ax.transAxes, fontsize=7, color='#888')
    plt.tight_layout()
...
<p><strong>Source unique :</strong> toutes les valeurs proviennent des CSV <code>formants_all_techniques_v2.csv</code> + <code>formants_yan_adds_v2.csv</code> (fallback legacy si absent).</p>
</div>
...
'''
    ...
    html += '''<p class="source-note"><strong>Source des données :</strong> formants_all_techniques_v2.csv + formants_yan_adds_v2.csv (fallback legacy si absent) — pipeline v22 validé.<br/>
...
def add_intro(doc):
    ...
    bullets = [
        ...
        ('Source unique : ', 'toutes les valeurs proviennent des CSV formants_all_techniques_v2.csv + formants_yan_adds_v2.csv (fallback legacy si absent).'),
    ]
...
def build_docx(output_path):
    ...
    p = doc.add_paragraph()
    notes = [
        'Source des données : formants_all_techniques_v2.csv + formants_yan_adds_v2.csv (fallback legacy si absent) — pipeline v22 validé.',
...
html_path = os.path.join(OUT_DIR, 'section_cuivres_v2.html')
docx_path = os.path.join(OUT_DIR, 'section_cuivres_v2.docx')
